  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          if (_userMemberRecord?.user_role == RoomMemberRoleEnum.owner.index)
            IconButton(
              icon: const Icon(
                Icons.edit_rounded,
                color: Colors.white,
      ...
      body: FutureBuilder(
        future: Future.wait([
          ...
          Room.getUserMemberRecord()
        ]),
        builder: (builder, AsyncSnapshot<List<dynamic>> snapshot) {
          if (snapshot.hasData) {
            List<RoomMember> members = snapshot.data![0];
            _userMemberRecord = snapshot.data![1];
            return Column(
              children: [
                Container(
                  height: 40,
